...
def harrison_ford
  ...
  Movie.joins(:actors).where(actors: {name: 'Harrison Ford'}).where('castings.ord != 1').select('movies.id, movies.title')
...
def biggest_cast
  ...
  Movie.joins(:castings).group('movies.id').select('movies.title, movies.id').order('COUNT(castings.actor_id) DESC').limit(3)
